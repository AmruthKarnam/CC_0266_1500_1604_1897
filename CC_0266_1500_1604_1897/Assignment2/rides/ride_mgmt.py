...
def createride():
    ...
    results=requests.get('http://172.255.0.3/api/v1/users')
    ...
    if count == 1:
        if str(ride['source']) in dict1.keys() and str(ride['destination']) in dict1.keys():
            ...
            ride['where']="1=1"
            ride['columns']='rideid'
            res=requests.post('http://localhost/api/v1/db/read', json = ride).json()
            #print(res)
            ride_count = 0
            for row in res :
                ride_count += 1
            ...
            res = requests.post('http://localhost/api/v1/db/write', json = ride)
            ride['isPut'] = 1
            ride['table'] = 'Riders'
...
def listupcomingride():
    ...
    res = requests.post('http://localhost/api/v1/db/read', json = res)
    #print(res)
    res = res.json()
    #print(res)
    #return
...
def listride(rideId):
    ...
    if count == 0 :
        ...
        ride['table']='Riders'
        res_rider = requests.post('http://localhost/api/v1/db/read', json = ride).json()
        for row in res_ride :
...
def joinride(rideId):
    ...
    print("1234")
    res= requests.post('http://localhost/api/v1/db/read', json = user)
    count = 0
    ride = {}
    for _ in res :
        count += 1
